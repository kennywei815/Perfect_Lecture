...
Function ReadCsv(code As String, table As Collection)
    ClearCollection table
    
    Lines = Split(code, vbNewLine)
    
    LinesLB = LBound(Lines)
    LinesUB = UBound(Lines)
    
    For i = LinesLB To LinesUB
        table.Add New Collection
        ...
        InField = False
        MergeWithPrev = False
        For j = LBound(Tuple) To UBound(Tuple)
            MergeWithPrev = InField
            
            If MergeWithPrev Then
                If Right(Tuple(j), 1) = """" Then
                    InField = False
                    Tuple(j) = Left(Tuple(j), Len(Tuple(j)) - 1)
                End If
            Else
                If Left(Tuple(j), 1) = """" Then
                    If Right(Tuple(j), 1) <> """" Then
                        InField = True
                        Tuple(j) = Right(Tuple(j), Len(Tuple(j)) - 1)
                    Else
                        Tuple(j) = Left(Tuple(j), Len(Tuple(j)) - 1)
                        Tuple(j) = Right(Tuple(j), Len(Tuple(j)) - 1)
                    End If
        ...
        '    If InField Then
        '        field = field & "," & Tuple(j)
        '    ElseIf MergeWithPrev Then
        '        field = field & "," & Tuple(j)
        '        table(idx).Add field
        '        field = ""
        '    Else
        '        table(idx).Add Tuple(j)
        '    End If
        '
        'Next j
...
Function WriteCsv(code As String, table As Collection)
    code = ""
    
    For i = 1 To table.Count
        If table(i).Count <> 0 Then
            If InStr(table(i).Item(1), ",") Then
                code = code & QuotedStr(table(i).Item(1))
            Else
                code = code & table(i).Item(1)
            End If
        End If
        
        For j = 2 To table(i).Count
            If InStr(table(i).Item(j), ",") Then
                code = code & "," & QuotedStr(table(i).Item(j))
            Else
                code = code & "," & table(i).Item(j)
            End If
        Next j
...
Function hasModule(name As String) As Boolean
    hasModule = False
